...
export const SelectedMusicList = ({ Musics, onRemove, onMove }: SelectedMusicListProps) => {
  return (
    <div className="flex-1 min-w-0 flex flex-col">
      ...
      <div className="flex-1 border-2 border-primary/20 rounded-xl overflow-y-auto max-h-[150px] p-2 bg-gray-4/50 custom-scrollbar space-y-2 min-h-[150px]">
        {Musics.length === 0 ? (
          ...
          Musics.map((Music, idx) => (
            <div key={Music.id} className="flex items-center bg-white p-2 rounded-lg border border-primary/10 shadow-sm">
              <img src={Music.coverUrl} alt="art" className="w-10 h-10 rounded border border-gray-3 object-cover flex-shrink-0" />

              <div className="ml-3 flex-1 min-w-0">
                <p className="text-sm font-bold truncate text-primary">{Music.title}</p>
                <p className="text-xs text-gray-1 truncate">{Music.artist}</p>
              </div>
